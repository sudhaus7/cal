...
class BaseView extends \TYPO3\CMS\Cal\Service\BaseService
{
    ...
    public function getTodoMarker(&$page, &$sims, &$rems, &$wrapped)
    {
        ...
        if ($this->rightsObj->isViewEnabled('todo') && $confArr['todoSubtype']=='todo') {
            ...
            $rems['###TODO###'] = \TYPO3\CMS\Cal\Utility\Functions::substituteMarkerArrayNotCached($todoTemplate, array(), array('###TODO_ENTRIES###'=>$todoContent));
        }
    }
    ...
    public function getUserLoginMarker(&$page, &$sims, &$rems, &$wrapped)
    {
        ...
        if ($this->conf['view.']['other.']['showLogin'] == 1) {
            ...
            $rems['###USER_LOGIN###'] = \TYPO3\CMS\Cal\Utility\Functions::substituteMarkerArrayNotCached($this->cObj->getSubpart($page, '###USER_LOGIN###'), $local_sims, $local_rems, array());
        }
    }
    ...
    public function getSearchMarker(&$page, &$sims, &$rems, &$wrapped)
    {
        ...
        if ($this->conf['view.']['other.']['showSearch'] == 1) {
            ...
            */
            ...
            $rems['###SEARCH###'] = \TYPO3\CMS\Cal\Utility\Functions::substituteMarkerArrayNotCached($this->cObj->getSubpart($page, '###SEARCH###'), $local_sims, array(), array());
        }
    }
    ...
    public function replace_files($page, $tags = array())
    {
        if (sizeof($tags) > 0) {
    ...
    public function tomorrows_events($template)
    {
        ...
        if (is_array($this->master_array[$next_day]) && sizeof($this->master_array[$next_day]) > 0) {
            ...
            return \TYPO3\CMS\Cal\Utility\Functions::substituteMarkerArrayNotCached($template, array(), $rems, array());
        }
